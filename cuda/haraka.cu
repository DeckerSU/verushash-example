...
__device__ __constant__ static const unsigned char haraka_rc[40][16] = {
...
__device__ static unsigned char rc[40][16];

/*
__device__ __constant__ static unsigned char rc[40][16] = {
...
__device__ static unsigned char rc_sseed[40][16];
...
__device__ void aesenc(unsigned char *s, const unsigned char *rk) 
{
    ...
    for (i = 0; i < 16; ++i) {
        s[i] = v[i / 4][i % 4] /*^ rk[i]*/; // VerusHash have 0 rc vector
    }
}
                     ...
                     unsigned long long seed_length)
{
    ...
    memcpy_decker((unsigned char *)rc, (unsigned char *)haraka_rc, 40*16);

    /* Constants for sk.seed */
    if (sk_seed != NULL) {
        haraka_S(buf, 40*16, sk_seed, seed_length);
        memcpy_decker((unsigned char *)rc_sseed, (unsigned char *)buf, 40*16);
    }

    /* Constants for pk.seed */
    haraka_S(buf, 40*16, pk_seed, seed_length);
    memcpy_decker((unsigned char *)rc, (unsigned char *)buf, 40*16);    
}

__device__ static void haraka_S_absorb(unsigned char *s, unsigned int r,
                            const unsigned char *m, unsigned long long mlen,
                            unsigned char p)
{
    unsigned long long i;
    //unsigned char t[r]; // haraka.cu(148): error: expression must have a constant value
    /* https://stackoverflow.com/questions/5531247/allocating-shared-memory/5531640#5531640 */

    printf("r = %d\n", r);
    unsigned char t[16384];
...
__device__ static void haraka_S_squeezeblocks(unsigned char *h, unsigned long long nblocks,
                                   unsigned char *s, unsigned int r)
{
    while (nblocks > 0) {
        haraka512_perm(s, s);
        memcpy_decker(h, s, HARAKAS_RATE);
        h += r;
        nblocks--;
    }
}
...
__device__ void haraka512_perm(unsigned char *out, const unsigned char *in) 
{
    ...
    /*
    memcpy_decker(s, (unsigned char *)in, 16);
    memcpy_decker(s + 16, (unsigned char *)in + 16, 16);
    memcpy_decker(s + 32, (unsigned char *)in + 32, 16);
    memcpy_decker(s + 48, (unsigned char *)in + 48, 16); */
...
__device__ void haraka256(unsigned char *out, const unsigned char *in) 
{
    ...
    memcpy_decker((unsigned char *)s, (unsigned char *)in, 16);
    memcpy_decker((unsigned char *)s + 16, (unsigned char *)in + 16, 16);
...
//__global__ void VerusHash_GPU(unsigned char *result, unsigned char *data, int len)
{
    ...
    /*
    int j;
    printf("[Decker] CVerusHash::Hash data = ");
    for (j=0; j<len; j++) {
	printf("%02x", ptr[j]);   	
    }
    printf("\n");
    */
    ...
    for ( ; pos < len; pos += 32)
    {
        if (len - pos >= 32)
        {
            memcpy_decker(bufPtr + 32, ptr + pos, 32);
        }
        else
        {
            int i = (int)(len - pos);
            memcpy_decker(bufPtr + 32, ptr + pos, i);
    ...
    memcpy_decker((unsigned char *)result, (unsigned char *)bufPtr, 32);

    /*
    printf("[Decker] CVerusHash::Hash result = ");
    for (j=0; j<32; j++) {
	printf("%02x", bufPtr[j]);   	
    }
    printf("\n");
